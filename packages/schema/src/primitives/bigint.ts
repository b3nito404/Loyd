...
class BigIntSchemaImpl extends BaseSchema<bigint> implements BigIntSchema {
  ...
  private _clone(rules: BigIntRule[]): BigIntSchema {
      ...
      (v) =>
        v < value
          ? this._fail(
              ...
              [],
              { min: value.toString(), actual: v.toString() },
              message
            )
          : null,
    ]);
      ...
      (v) =>
        v > value
          ? this._fail(
              ...
              [],
              { max: value.toString(), actual: v.toString() },
              message
            )
          : null,
    ]);
  ...
  positive(message?: string): BigIntSchema {
    return this._clone([
      ...this._rules,
      (v) => (v <= 0n ? this._fail("ERR_NUMBER_NOT_POSITIVE", [], {}, message) : null),
    ]);
  }
  negative(message?: string): BigIntSchema {
    return this._clone([
      ...this._rules,
      (v) => (v >= 0n ? this._fail("ERR_NUMBER_NOT_NEGATIVE", [], {}, message) : null),
    ]);
  }
  nonnegative(message?: string): BigIntSchema {
    return this._clone([
      ...
      (v) =>
        v < 0n
          ? this._fail("ERR_NUMBER_TOO_SMALL", [], { min: "0", actual: v.toString() }, message)
          : null,
    ]);
  }
  multipleOf(value: bigint, message?: string): BigIntSchema {
      ...
      (v) =>
        v % value !== 0n
          ? this._fail(
              ...
              [],
              { multipleOf: value.toString(), actual: v.toString() },
              message
            )
          : null,
    ]);
...
export function bigint(message?: string): BigIntSchema {
  return new BigIntSchemaImpl(message, []);
}
