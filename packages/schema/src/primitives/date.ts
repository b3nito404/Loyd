...
class DateSchemaImpl extends BaseSchema<Date> implements DateSchema {
  ...
  private _clone(rules: DateRule[]): DateSchema {
  ...
  min(date: Date, message?: string): DateSchema {
    return this._clone([
      ...this._rules,
      (v) =>
        v < date ? this._fail("ERR_DATE_TOO_EARLY", [], { min: date, actual: v }, message) : null,
    ]);
  }
  max(date: Date, message?: string): DateSchema {
    return this._clone([
      ...this._rules,
      (v) =>
        v > date ? this._fail("ERR_DATE_TOO_LATE", [], { max: date, actual: v }, message) : null,
    ]);
  }
  past(message?: string): DateSchema {
...
export function date(message?: string): DateSchema {
  return new DateSchemaImpl(message, []);
}
