...
class RecordSchemaImpl<K extends string, V> extends BaseSchema<Record<K, V>> implements RecordSchema<K, V> {
  readonly _type = "record" as const;
  constructor(private readonly _key: LoydSchema<K> | null, private readonly _value: LoydSchema<V>, private readonly _msg?: string) { super(); }
  ...
  _validate(input: unknown): LoydResult<Record<K, V>> {
    ...
    for (const k of Object.keys(raw)) {
      if (this._key) {
        ...
        if (!kr.success) { for (const iss of kr.issues) issues.push({ ...iss, path: [k, ...iss.path] }); continue; }
      }
...
export function record<K extends string, V>(keyOrValue: LoydSchema<K> | LoydSchema<V>, value?: LoydSchema<V>): RecordSchema<K, V> {
