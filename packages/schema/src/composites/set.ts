...
class SetSchemaImpl<T> extends BaseSchema<Set<T>> implements SetSchema<T> {
  ...
  min(size: number, message?: string): SetSchema<T> {
    return new SetSchemaImpl(this._v, size, this._max, message);
  }
  max(size: number, message?: string): SetSchema<T> {
    return new SetSchemaImpl(this._v, this._min, size, message);
  }
  nonempty(message?: string): SetSchema<T> {
    return this.min(1, message);
  }
}
export function set<T>(v: LoydSchema<T>, message?: string): SetSchema<T> {
  return new SetSchemaImpl(v, undefined, undefined, message);
}
