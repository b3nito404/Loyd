...
export const en: MessageMap = {
  ...
  [LoydErrorCode.INVALID_TYPE]:          ({ expected, received }) => `Expected ${expected}, received ${received}`,
  [LoydErrorCode.STRING_TOO_SHORT]:      ({ min, actual }) => `Must be at least ${min} characters (got ${actual})`,
  [LoydErrorCode.STRING_TOO_LONG]:       ({ max, actual }) => `Must be at most ${max} characters (got ${actual})`,
  ...
  [LoydErrorCode.NUMBER_TOO_SMALL]:      ({ min, inclusive }) => `Must be ${inclusive ? ">=" : ">"} ${min}`,
  [LoydErrorCode.NUMBER_TOO_LARGE]:      ({ max, inclusive }) => `Must be ${inclusive ? "<=" : "<"} ${max}`,
